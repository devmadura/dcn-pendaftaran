...
import { IconChevronDown, IconLoader2, IconCheck, IconX } from "@tabler/icons-react";
...
export function BiodataForm({ isProfileComplete, onSuccess }: BiodataFormProps) {
  const [formNim, setFormNim] = useState("");
  const [formAngkatan, setFormAngkatan] = useState("");
  const [formLoading, setFormLoading] = useState(false);
  const [formErrors, setFormErrors] = useState<{ nim?: string; angkatan?: string }>({});
  ...
  if (isProfileComplete) {
    return (
      <div className="mt-4 p-8 border border-white/5 bg-white/5 rounded-[24px] text-center">
        ...
        <h3 className="text-lg font-semibold text-white/90">Data Profil Sudah Lengkap</h3>
        <p className="text-sm text-white/50 max-w-md mx-auto mt-2">NIM dan Tahun Angkatan sudah tersimpan di sistem. Tim kami akan segera memproses verifikasi berkas kamu.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 p-6 sm:p-8 glass-card border border-white/10 rounded-[24px]">
      <h3 className="text-xl font-semibold mb-2 text-white/90">Lengkapi Biodata</h3>
      <p className="text-sm text-white/50 mb-8">Data ini wajib diisi sesuai dengan identitas akademik Universitas Madura.</p>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label htmlFor="formNim" className="block text-sm font-medium text-white/80">
            ...
            <div className="relative">
              <input
                ...
                onBlur={() => setFormErrors(p => ({ ...p, nim: validateNIM(formNim) || undefined }))}
                className={cn("form-input w-full", formErrors.nim && "form-input-error")}
            ...
            {formErrors.nim && <p className="text-xs text-red-400">{formErrors.nim}</p>}
            {nimValidity === 'valid' && studentName && (
              <p className="text-xs text-green-400 font-medium">✨ {studentName}</p>
            )}
            {nimValidity === 'invalid' && !isValidating && formNim.length >= 10 && (
              <p className="text-xs text-red-400">NIM tidak terdaftar di sistem UNIRA</p>
            )}
          </div>

          <div className="space-y-2 relative">
            <label htmlFor="formAngkatan" className="block text-sm font-medium text-white/80">
            ...
            <div className="relative">
              <input
                ...
                value={formAngkatan}
                readOnly
                className={cn("form-input w-full bg-black/30 text-white/70 cursor-not-allowed", formErrors.angkatan && "form-input-error")}
                placeholder="Terisi otomatis dari NIM"
              />
            </div>
            {formErrors.angkatan && <p className="text-xs text-red-400">{formErrors.angkatan}</p>}
          </div>
        </div>
